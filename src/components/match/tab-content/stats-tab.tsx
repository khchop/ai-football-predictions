...
export function StatsTab({
  ...
  const h2hResults = analysis?.h2hResults
    ? (JSON.parse(analysis.h2hResults) as H2HMatch[])
    : [];
  ...
  return (
    <div className="space-y-6">
          ...
          {isCupMatch ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="font-bold text-sm">
                    {homeStanding?.teamName || 'Home Team'}
                  </span>
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="font-bold text-sm">
                    {awayStanding?.teamName || 'Away Team'}
                  </span>
                  <span className="text-xs text-muted-foreground">
      ...
      {/* H2H History - Progressive disclosure on mobile */}
                    ...
                    >
                      ...
                      <div className="flex-1 flex items-center justify-center gap-3">
                        <span
                          className={cn(
                            'font-medium',
                            m.homeScore > m.awayScore && 'text-primary'
                          )}
                        >
                          {m.homeTeam}
                        </span>
                        <span className="font-bold bg-muted px-2 py-0.5 rounded">
                          {m.homeScore}-{m.awayScore}
                        </span>
                        <span
                          className={cn(
                            'font-medium',
                            m.awayScore > m.homeScore && 'text-primary'
                          )}
                        >
                          {m.awayTeam}
                        </span>
                ...
                <div className="pt-2 text-xs text-center text-muted-foreground">
                  ...
                  {(analysis.h2hHomeWins ?? 0) > (analysis.h2hAwayWins ?? 0)
                    ...
                    : (analysis.h2hAwayWins ?? 0) > (analysis.h2hHomeWins ?? 0)
                      ...
                      : ' - All square'}
                  {(analysis.h2hDraws ?? 0) > 0 &&
                    ` with ${analysis.h2hDraws} draw${(analysis.h2hDraws ?? 0) > 1 ? 's' : ''}`}
                </div>
              )}
            </CardContent>
          </Card>
      ...
      {/* H2H History - Always visible on desktop */}
      ...
      {/* Match Predictions - Always visible */}
      <Card className="bg-card/50 border-border/50">
        <CardContent className="p-6 space-y-6">
          <h3 className="text-lg font-bold flex items-center gap-2">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div>
                    ...
                    <p className="text-sm font-bold">
                      {(parseFloat(analysis.oddsHome) || 0).toFixed(2)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Draw</p>
                    <p className="text-sm font-bold">
                      {(parseFloat(analysis.oddsDraw) || 0).toFixed(2)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Away</p>
                    <p className="text-sm font-bold">
                      {(parseFloat(analysis.oddsAway) || 0).toFixed(2)}
                    </p>
                  </div>
                </div>
              </div>
