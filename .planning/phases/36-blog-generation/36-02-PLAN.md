...
must_haves:
  truths:
    ...
    - "Blog content is sanitized before save"
  artifacts:
...
5. BLOG-03 VERIFICATION: Verify that `sanitizeContent` is called in both blog functions before database save:
   - In `generateLeagueRoundup`: Confirm sanitizeContent is called on LLM output (around lines 384-396)
   - In `generateModelReport`: Confirm sanitizeContent is called on LLM output (around lines 478-490)
   - Run: `grep -n "sanitizeContent" src/lib/content/generator.ts | grep -E "generateLeagueRoundup|generateModelReport" -A 20`
   - If sanitization is missing, add: `const sanitized = sanitizeContent(llmOutput);` before database save
  </action>
  <verify>
...
4. Verify sanitization in blog functions: `grep -B 5 -A 5 "sanitizeContent" src/lib/content/generator.ts | grep -E "Roundup|Report"`
  </verify>
  <done>
generateLeagueRoundup and generateModelReport throw RetryableContentError on LLM failures, FatalContentError on validation failures, and apply sanitizeContent before database save
  </done>
</task>

<task type="auto">
  <name>Task 2: Verify blog job error propagation to worker and DLQ</name>
...
NOTE: DLQ routing was configured in Phase 32 content queue setup. The content worker inherits DLQ configuration from the queue setup - when errors propagate (are thrown), BullMQ automatically routes failed jobs to the DLQ after retry exhaustion per the removeOnFail settings. No additional DLQ code needed in worker.
  </action>
  <verify>
...
3. Test TypeScript compilation: `npx tsc --noEmit`
  ...
  <done>
Content worker properly handles 'league_roundup' and 'model_report' job types, errors propagate to BullMQ for retry logic and DLQ routing (inherited from Phase 32 queue setup)
  </done>
</task>

<task type="auto">
  <name>Task 3: Verify job schedulers are registered (verification only)</name>
  <files>src/lib/queue/setup.ts</files>
  <action>
This is a VERIFICATION-ONLY task. Per the research findings, blog job schedulers already exist in setup.ts. Do NOT add new schedulers - only verify existing ones are correctly configured.

1. Verify weekly league roundup scheduler exists (should be around lines 308-320):
   - Job name: 'scan-league-roundups'
   - Pattern: '0 8 * * 1' (Monday 8 AM) or similar weekly pattern
   - Data type: 'scan_league_roundups'

2. Verify monthly model report scheduler exists (should be around lines 322-339):
   - Job name: 'generate-model-report'
   - Pattern: '0 9 1 * *' (1st of month 9 AM) or similar monthly pattern
   - Data type: 'model_report'

3. Document findings:
   - If schedulers exist and are correct: Log "Verified: schedulers already registered correctly"
   - If schedulers exist but have issues: Document the specific issues found
   - If schedulers are missing: STOP and report as a blocker (this would indicate prior implementation is incomplete)

Expected outcome: Schedulers already exist from prior implementation. This task confirms they are properly configured.
  </action>
  <verify>
...
3. Verify cron patterns are valid: Check patterns against standard cron format
  ...
  <done>
Verified: Blog job schedulers are registered with correct cron patterns and job types. Weekly roundups trigger Monday 8 AM, monthly reports trigger 1st of month 9 AM.
  </done>
</task>
...
5. Verify sanitization in blog functions: `grep -c "sanitizeContent" src/lib/content/generator.ts` (should show calls in blog functions)
</verification>
...
3. Blog content is sanitized via sanitizeContent before database save
4. Content worker re-throws errors (does not suppress with return false)
5. Job schedulers for league roundups (weekly) and model reports (monthly) are registered
