...
must_haves:
  truths:
    ...
    - "Queue metrics logging (Pino) includes matchesWithoutPredictions count alongside existing queue stats"
  artifacts:
...
<task type="auto">
  <name>Task 2: Add pipeline health alerts to backfill worker (MON-03)</name>
...
This is wrapped in its own try/catch so a health check failure never blocks the actual backfill work.
  ...
  <verify>
1. `npx tsc --noEmit` passes (no type errors in backfill.worker.ts)
2. Import for pipeline-coverage module resolves correctly
3. Health check is AFTER step 6 and inside its own try/catch
  </verify>
  <done>
Backfill worker logs pipeline coverage at INFO level and critical gaps at ERROR level on every hourly cycle (MON-03). Uses shared getMatchCoverage() from Plan 01.
  </done>
</task>

<task type="auto">
  <name>Task 3: Extend queue metrics with matchesWithoutPredictions (MON-04)</name>
  <files>src/lib/logger/metrics.ts</files>
  <action>
...
Use `debug` level for the error case since periodic metrics logging failure is not critical.
  </action>
  <verify>
1. `npx tsc --noEmit` passes (no type errors in metrics.ts)
2. getMatchCoverage import resolves correctly
3. New coverage metrics block is AFTER the existing queue metrics logging
  </verify>
  <done>
Queue metrics include matchesWithoutPredictions count in periodic 5-minute logging (MON-04). Uses shared getMatchCoverage() from Plan 01.
  </done>
</task>
