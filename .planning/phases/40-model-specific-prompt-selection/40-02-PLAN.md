...
2. **Add concrete optional property** to OpenAICompatibleProvider class (NOT abstract - subclasses optionally override):
```typescript
// Model-specific configuration (optional, subclasses can override)
protected promptConfig?: PromptConfig;
```
NOTE: Do NOT use `abstract` keyword. The property is optional - subclasses that don't need configuration simply don't override it (undefined = use defaults). Abstract would require ALL subclasses to implement, which is wrong for optional configuration.
...
3. **Update callAPI method** to:
   ...
   c. **Unify content extraction and apply response handler** (lines ~300-320):

   The current code has 3 separate return paths:
   - `return message.content;` (line ~304)
   - `return message.reasoning;` (line ~309)
   - `return detail.summary;` (line ~316)

   **REFACTOR to unified pattern:** Replace all 3 return paths with:
   ```typescript
...
```
No errors. Property (not abstract) and imports present.
  </verify>
  <done>
...
- promptConfig is concrete optional property (NOT abstract)
- callAPI applies prompt variant to system prompt
- callAPI uses model-specific timeout when configured
- callAPI extracts content to unified variable, then applies handler, then returns
- Single return path with response handler applied
