...
2. Update LeaderboardEntry interface - add scoredPredictions if not present:
   - The leaderboard API returns totalPredictions which for scored entries equals scoredPredictions
   - Verify this by checking that leaderboard query uses `status = 'scored'` filter
...
3. Update the accuracy column (column id 'accuracy', around line 294-322):
   ...
   cell: ({ row }) => {
     ...
     const total = entry.totalPredictions; // This is scoredPredictions from the query
...
4. Tooltip links to /methodology page for full explanation
5. Build passes and no visual regressions
</success_criteria>
