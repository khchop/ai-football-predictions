...
<task type="auto">
  ...
  <action>
    **In `backfill.worker.ts`:**
    ...
    5. **Add a summary of the chain status** to the backfill return value. After the existing results object, add a `windows` field:
       ```typescript
       const results = {
         analysisTriggered: 0,
         oddsTriggered: 0,
...
<task type="auto">
  ...
  <action>
    Verify all Phase 49 changes compile and build cleanly:

    1. **Type check:** Run `npx tsc --noEmit` and verify zero errors.

    2. **Production build:** Run `npx next build --webpack` and verify successful compilation. If the build fails, investigate and fix the root cause (likely a type error from the changes in Plan 01 or Task 1).
  </action>
  <verify>
    `npx tsc --noEmit` exits 0.
    `npx next build --webpack 2>&1 | tail -5` shows successful build.
  </verify>
  <done>
    Type checking and production build both pass with all Phase 49 changes applied.
  </done>
</task>
...
<verification>
After both tasks complete, verify all 5 PIPE requirements:

1. `npx tsc --noEmit` passes with zero errors
2. `npx next build --webpack` succeeds
3. PIPE-01: scheduleMatchJobs handles past-due matches (no early exit on kickoff <= now, status guard present, shouldRun logic preserved with kickoff > now)
4. PIPE-02: Fixtures worker schedules all scheduled matches (not just new), IndexNow only for new
