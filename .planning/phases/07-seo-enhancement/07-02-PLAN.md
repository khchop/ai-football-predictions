...
<objective>
Add structured data to blog roundups and competition pages for search engine understanding.

Purpose: Blog roundups about matchdays benefit from Article + ItemList schema (list of matches covered). Competition pages need SportsOrganization schema to establish entity recognition in Google's Knowledge Graph.

Output: Enhanced blog and competition pages with appropriate schema markup and optimized metadata.
</objective>
...
2. Update `src/app/blog/[slug]/page.tsx`:
   - For `contentType === 'league_roundup'`:
     - Query matches covered by this roundup (if match IDs stored in post metadata)
     - If no match data available, build ItemList from markdown content parsing (extract team names)
     - Use buildRoundupSchema instead of generic generateArticleSchema
...
<task type="auto">
  <name>Task 3: Ensure unique titles across all page types</name>
  <files>
    src/lib/seo/metadata.ts
...
4. Unique titles audit: `grep -rhn "title:" src/app/**/page.tsx | sort | uniq -d` returns empty (no duplicates)
</verification>
