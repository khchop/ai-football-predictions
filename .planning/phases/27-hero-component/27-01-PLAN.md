...
must_haves:
  truths:
    ...
    - "LIVE badge has NO animation/pulsing (user decision)"
  artifacts:
...
**CRITICAL - User Decision Compliance (from 27-CONTEXT.md):**
The existing Badge component has `animate-pulse` on variant="live", but the user explicitly decided:
"Red 'LIVE' badge - classic solid red, no pulsing/animation"

When using Badge variant="live", you MUST override the animation:
```tsx
<Badge variant="live" className="animate-none">LIVE</Badge>
```

This ensures the LIVE badge is static (no pulsing) per user decision.

Important per user decisions:
- NO venue in hero (keep minimal)
- NO form or league position
- NO animation/pulsing on LIVE badge (user rejected) - use className="animate-none" override
...
Export structure: `export function MatchHero() { ... }` (named export)
  </action>
  <verify>
TypeScript compiles:
```bash
...
Verify no animation on LIVE badge:
```bash
grep -q 'animate-none' src/components/match/match-hero.tsx && echo "PASS: LIVE badge animation disabled" || echo "FAIL: Missing animate-none override"
```
  </verify>
  <done>
...
- LIVE badge has NO animation (uses animate-none override per user decision)
...
4. LIVE badge has no animation (user decision compliance):
```bash
grep -q "animate-none" src/components/match/match-hero.tsx && echo "PASS: LIVE badge static" || echo "FAIL: Animation not disabled"
```

5. No duplicate score display patterns created (hero is single source)
</verification>
...
- LIVE badge has NO animation/pulsing (animate-none class applied)
