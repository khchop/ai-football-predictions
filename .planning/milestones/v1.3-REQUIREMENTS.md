# Requirements Archive: v1.3 Match Page Refresh
...
- [x] **PERF-01**: Match pages use ISR with appropriate revalidation (60s for all statuses)
- [x] **PERF-02**: Match data, predictions, and content fetched in parallel with Promise.all
- [x] **PERF-03**: TTFB under 400ms on mobile networks (infrastructure in place)

### AI Search Optimization

- [x] **SRCH-01**: robots.txt allows AI crawlers (GPTBot, ClaudeBot, PerplexityBot, Amazonbot)
- [x] **SRCH-02**: llms.txt file provides structured content paths for AI training/RAG
- [x] **SRCH-03**: Single consolidated Schema.org JSON-LD graph per match page (SportsEvent + WebPage + BreadcrumbList)
- [x] **SRCH-04**: AI-generated content visible to crawlers (not hidden behind client-side rendering)
...
**Adjusted:**
- PERF-01 adjusted from conditional revalidation (60s live, 300s scheduled, 3600s finished) to single 60s for all statuses (Next.js limitation)
- PERF-03 adjusted from guaranteed <400ms to "infrastructure in place" (production measurement required)
